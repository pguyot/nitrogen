...
new_a_1() ->
  Rec_a = #a{},
  iolist_to_binary(element_a:render("1",Rec_a)).

new_a_2() ->
  Rec_a = #a{postback="undefined"},
  iolist_to_binary(element_a:render("2",Rec_a)).

new_a_3() ->
  Rec_a = #a{postback="Postback"},
  iolist_to_binary(element_a:render("3",Rec_a)).

new_a_4() ->
  Rec_a = #a{body=#image { image="/path/to/image.gif" }},
  iolist_to_binary(element_a:render("4",Rec_a)).

new_a_5() ->
  Rec_a = #a{body="A LINK BODY", text="LINK TEXT", html_encode=true, href="not_javascript", postback="mypostback"},
  iolist_to_binary(element_a:render("5",Rec_a)).
