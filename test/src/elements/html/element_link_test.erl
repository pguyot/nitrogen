...
new_link_1() ->
    Rec_link = #link{},
    iolist_to_binary(element_link:render("1",Rec_link)).

new_link_2() ->
    Rec_link = #link{postback="undefined"},
    iolist_to_binary(element_link:render("2",Rec_link)).

new_link_3() ->
    Rec_link = #link{postback="Postback"},
    iolist_to_binary(element_link:render("3",Rec_link)).

new_link_4() ->
    Rec_link = #link{body=#image { image="/path/to/image.gif" }},
    iolist_to_binary(element_link:render("4",Rec_link)).

new_link_5() ->
    Rec_link = #link{body="A LINK BODY", text="LINK TEXT", html_encode=true, url="not_javascript", postback="mypostback"},
    iolist_to_binary(element_link:render("5",Rec_link)).
